...
def get_output_path(year, month):
    ...
    if S3_ENDPOINT_URL is None:
        default_output_pattern = 's3://nyc-duration-hoe/taxi_type=fhv/year={year:04d}/month={month:02d}/predictions.parquet'
        output_pattern = os.getenv('OUTPUT_FILE_PATTERN', default_output_pattern)
        return output_pattern.format(year=year, month=month)
    else:
        default_output_pattern = 's3://nyc-duration/out_{year:04d}-{month:02d}.parquet'
        output_pattern = os.getenv('OUTPUT_FILE_PATTERN', default_output_pattern)
        return output_pattern.format(year=year, month=month)
